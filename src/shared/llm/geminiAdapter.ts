...
export const geminiInteractionsAdapter: LLMAdapter = {
  async request(config: ProviderConfig, request: LLMRequest): Promise<LLMResponse> {
    ...
    const body = {
      ...
      ...(Object.keys({ ...parameters, ...generationConfig }).length > 0
        ? { generation_config: { ...parameters, ...generationConfig } }
        : {}),
      ...(request.jsonMode
        ? {
            response_format: {
    ...
    const data = await sendLLMRequest<any>(
      buildEndpoint(config.baseUrl || 'https://generativelanguage.googleapis.com/v1beta'),
      {
        method: 'POST',
