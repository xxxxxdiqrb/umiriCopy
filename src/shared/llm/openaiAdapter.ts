...
export const openAICompatibleAdapter: LLMAdapter = {
  async request(config: ProviderConfig, request: LLMRequest): Promise<LLMResponse> {
    const body = {
      ...
      ...(request.jsonMode
        ? request.jsonSchema
          ? { response_format: { type: "json_schema", json_schema: { name: "translation_result", strict: true, schema: request.jsonSchema } } }
...
export const openAIResponsesAdapter: LLMAdapter = {
  async request(config: ProviderConfig, request: LLMRequest): Promise<LLMResponse> {
    ...
    const body = {
      ...
      ...(request.jsonMode
        ? request.jsonSchema
          ? { text: { format: { type: "json_schema", name: "translation_result", strict: true, schema: request.jsonSchema } } }
