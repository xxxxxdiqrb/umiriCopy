...
async function getInstagramImageUrlList(
  article: HTMLElement,
  isDetail: boolean,
): Promise<string[]> {
  let ul: HTMLElement | null = null, scrollElement: HTMLElement | null = null;
  if (!isDetail) {
    const carouselContainer = article.querySelector(
    ...
    if (!ul || !scrollElement) {
      return Array.from(carouselContainer.querySelectorAll("img")).map(
        (item) => item.src,
      );
    }
  } else {
    ...
    if (!ul || !scrollElement) {
      return Array.from(article.querySelectorAll("img[srcset]")).map(
...
async function getInstagramCaption(
  article: HTMLElement,
  isDetail: boolean,
): Promise<string> {
  if (isDetail) {
    return article.querySelector("h1")?.innerText || "";
  } else {
    const moreTextButton = article.querySelector(
      'span > div[aria-disabled="false"]',
    ) as HTMLElement | null;
    if (moreTextButton) {
  ...
  if (platformState.configBar.copyImages) {
    ...
    for (let i = 0; i < imageUrlList.length; i++) {
      ...
      const result = await processImage(
        { name: `${instagramName}_${i}.jpg`, url: imageUrlList[i] },
