...
import { processImage, formatImageHtml, sleep, translateTextContents, CopyStageError, toCopyStageError } from "../../../shared/utils";
...
async function getAltText(presentation: Element): Promise<string> {
  try {
    const altButton = Array.from(presentation.querySelectorAll("span")).find((item) => item.textContent === "ALT");
    if (!altButton) return "";
    await displayDialog();
    altButton.click();
...
async function displayDialog() {
  ...
  const clickTarget = Array.from(parent.children).find((item): item is HTMLElement => item !== hoverCard && item instanceof HTMLElement);
  if (!clickTarget) {
    return;
  }
  clickTarget.click();
...
async function getImageEntryList(article: HTMLElement, includeAlt = platformState.configBar.getAlt): Promise<TweetImageEntry[]> {
...
async function extractTweetImages(article: HTMLElement, tweetName: string, startIndex: number, totalCount: number): Promise<string[]> {
  ...
  if (platformState.configBar.getAlt && platformState.configBar.translate && altTexts.some((text) => text.trim())) {
  ...
  for (let entryIndex = 0; entryIndex < imageEntryList.length; entryIndex++) {
    ...
    let result;
    try {
      result = await processImage({ name: `${tweetName}_${baseUrl.split("/").pop()}.jpg`, url: imgUrl }, platformState.configBar.download);
    } catch (error) {
