# umiriCopy

<p align="center">
  <img src="public/icons/icon.jpg" alt="umiriCopy Logo" height="120">
</p>

一个 Edge/Chrome 浏览器扩展，用于将 X (Twitter) 推文和 Instagram 帖子一键复制到剪贴板，支持翻译、图片复制和视频下载，适用于 QQ 群进行 DD。

## 功能特性

- **多平台支持** - 支持 X (Twitter)、Instagram 和 TikTok（仅视频下载）
- **一键复制** - 快速将推文/帖子内容复制到剪贴板
- **AI 翻译** - 支持自行配置 OpenAI 兼容的 LLM 翻译服务
- **批量选择** - 支持选择多条推文合并复制（Twitter）
- **图片复制** - 支持复制推文/帖子中的图片到剪贴板
- **视频下载** - 支持 X (Twitter)、Instagram 和 TikTok 的视频下载

## ⚠️ 注意

本项目由 AI 辅助编写，有可能会有未检查出来的💩！

## 图标来源

图标来源: [https://x.com/ShaoRouHong](https://x.com/ShaoRouHong)

Shao老师的画太可爱了你知道吗

## TODO List

- [x] 支持 Instagram 帖子复制
- [x] 支持视频下载
- [x] 支持配置导入与导出
- [ ] 支持 bilibili 动态复制
- [ ] 支持 新浪微博 动态复制

## 安装

### 直接安装

在 Release 中下载最新版本的 zip 文件，加载到 Edge/Chrome 浏览器：

1. 打开浏览器，访问扩展管理页面（如 `edge://extensions/` 或 `chrome://extensions/`）
2. 开启「开发人员模式」
3. 将文件拖到浏览器中安装

### 从源码构建

```bash
# 安装依赖
npm install

# 构建
npm run build
```

构建完成后，将 `dist` 目录加载到浏览器：

1. 打开浏览器，访问扩展管理页面
2. 开启「开发人员模式」
3. 点击「加载解压缩的扩展」
4. 选择 `dist` 目录

## 使用说明

### 基本使用

#### 复制推文/帖子 (X / Instagram)

1. 访问 [X (Twitter)](https://x.com) 或 [Instagram](https://www.instagram.com)
2. 点击页面右上方的悬浮按钮进入选择模式，点击推文/帖子选中
3. 在配置栏中选择是否翻译、是否复制图片等选项
4. 点击「复制」按钮完成复制，或选择带有视频的帖子后点击「下载视频」并等待视频下载完成即可

#### 下载视频

1. 访问 [TikTok](https://www.tiktok.com) 的视频详情页，比如 `https://www.tiktok.com/@liyuu_official/video/7623714955866492180`
2. 点击页面右上方的悬浮按钮按钮，弹出确认栏后点击「下载视频」即可

### 配置管理

1. 点击浏览器工具栏中的扩展图标
2. 点击「翻译配置」进入配置页面
3. 在页面右上角点击「导出配置」可以备份当前全部设置，或点击「导入配置」快速导入已有 JSON 配置文件
4. 从导航栏切换到 **平台默认配置** 中可独立设置 X (Twitter) 与 Instagram 的对应选项的默认行为

## 项目结构

```
umiriCopy/
├── src/
│   ├── platforms/     # 各平台 Content Script 实现
│   ├── shared/        # 共享组件、状态与工具
│   ├── background/    # Service Worker
│   ├── options/       # 扩展配置页面
│   ├── popup/         # 扩展弹窗
│   └── manifest.json  # 扩展清单
├── options/           # Options HTML 入口
├── popup/             # Popup HTML 入口
├── public/            # 静态资源
└── scripts/           # 构建脚本
```

## 技术栈

- **框架**: Vue 3 + TypeScript
- **构建工具**: Vite
- **样式**: SCSS
- **图片处理**: html-to-image

## 许可证

[MIT License](LICENSE)
